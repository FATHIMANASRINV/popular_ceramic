...
<div class="w-full px-6 py-6 mx-auto">
	<div class="max-w-md bg-white shadow-lg rounded-2xl p-6">
		<form method="GET" action="{{ route(auth()->user()->user_type.'.inventory.pending_sales') }}">
			...
			<div class="w-1/2">
				...
				<option value="hold" selected>Pending</option>
					...
					<tbody>
						...
						<tr class="hover:bg-gray-50">
							...
							<td class="border border-gray-300 px-4 py-2">
								@if($Products->status=='hold')
								<a class="approve-sales" data-id="{{  $Products->id  }}">
									<i class="fas fa-check text-blue-500 cursor-pointer" style="color: green;"></i></a>
									&nbsp;&nbsp;
									<a class="delete-sales" data-id="{{  $Products->id  }}"><i class="fas fa-trash text-red-500 cursor-pointer"></i></a>
									@else
									---
									@endif
								</td>
...
<script>
	...
	$(document).on('click', '.approve-sales', function(e) {
		e.preventDefault(); 
		var id = $(this).data('id');
		$.ajax({
			url: '{{ route(auth()->user()->user_type.".inventory.approve_sales") }}',
			method: "POST",
