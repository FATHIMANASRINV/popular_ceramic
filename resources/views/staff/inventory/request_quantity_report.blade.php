...
@section('title', 'Requested Quantity Report')
@section('header')
<style>
	.modal-overlay {
		position: fixed;
...
<div class="w-full px-6 py-6 mx-auto">
	<div class="max-w-md bg-white shadow-lg rounded-2xl p-6">
		<form method="GET" action="{{ route(auth()->user()->user_type.'.inventory.request_quantity_report') }}">
				...
				<table class="w-full border-collapse border border-gray-300 text-slate-600 rounded-lg shadow">
					...
					<tbody>
						...
						<tr class="hover:bg-gray-50">
							...
							@if(auth()->user()->user_type=='admin')
							<td class="border border-gray-300 px-4 py-2"><a class="approve-quantity" data-id="{{  $Products->id  }}">
								<i class="fas fa-check text-blue-500 cursor-pointer" style="color: green;"></i></a>
								&nbsp;&nbsp;
								<a class="delete-quantity" data-id="{{  $Products->id  }}"><i class="fas fa-trash text-red-500 cursor-pointer"></i></a></td>
								@endif
								@endforeach
							</tbody>
						</table>
